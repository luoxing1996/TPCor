...
r0 = ggplot(data,aes(protein_log2FC,-1*log10(protein_FDR)))
r1=r0+theme_bw()+theme(panel.grid=element_blank(),panel.border=element_blank(),axis.line=element_line(size=1,colour="black"))
r2 =r1+ geom_point(aes(color =protein_type))
r3 = r2 +geom_point(aes(color =protein_type))
r4=r3 + labs(title="protein",x="log2(ratio)",y="-log10(P_value)")+theme(title=element_text(family="myFont",size=20,
                           face="italic",hjust=0.2,lineheight=0.2))+theme(axis.text.x=element_text(size=15))+theme(axis.text.y=element_text(size=15))
r5 = r4 +scale_color_manual(values = c("#00ba38","#619cff","#f8766d"))
